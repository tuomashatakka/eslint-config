...
export default {
  ...
  create (context) {
    ...
    function getMemberName (node) {
      if (node.type === 'MethodDefinition' || node.type === 'ClassProperty' ||
        ...
        if (node.key.type === 'Identifier')
          return node.key.name; else if (node.key.type === 'Literal')
    ...
    function checkGroupPadding (members) {
      if (paddingBetweenGroups <= 0 || members.length < 2)
        return
      ...
      for (let i = 1; i < members.length; i++) {
        ...
        if (!prevMember.group || !currentMember.group)
          continue
